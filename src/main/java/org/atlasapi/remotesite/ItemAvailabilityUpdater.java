...
public class ItemAvailabilityUpdater implements Runnable {
	...
	public ItemAvailabilityUpdater(DatabasedMongo db, AdapterLog log) {
		this(db, log, new SystemClock());
	}
	...
	private DBObject updateAvailabilityOf(DBObject dbo, DateTime now) {
		return translator.toDBObject(null, updateAvailabilityOf(translator.fromDBObject(dbo, null), now));
	}
