...
private final static Daily AT_NIGHT = RepetitionRules.daily(new LocalTime(5, 0, 0));
    ...
    public @Bean HboSiteMapUpdater siteMapUpdater() {
        return new HboSiteMapUpdater(HttpClients.screenScrapingClient(), brandAdapter(), contentStore, log);
    }
    ...
    public @Bean HboItemAdapter episodeAdapter() {
        return new HboItemAdapter(HttpClients.screenScrapingClient(), log, hboAdapterHelper());
    }
    
    public @Bean HboBrandAdapter brandAdapter() {
        return new HboBrandAdapter(episodeAdapter(), HttpClients.screenScrapingClient(), log, hboAdapterHelper());
    }
    
    public Collection<SiteSpecificAdapter<? extends Content>> adapters() {
        return ImmutableList.<SiteSpecificAdapter<? extends Content>>of(brandAdapter());
    }
}
