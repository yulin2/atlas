...
public class SeesawContentProcessor {
    ...
    private Map<String, Identified> idToDescription = Maps.newHashMap();
    private Map<Identified, String> descriptionToParentId = Maps.newHashMap();
    ...
    public void joinContent() {
        for (Episode episode : allEpisodes) {
            ...
            Identified description = idToDescription.get(parentId);
            if (description != null) {
                if (description instanceof Series) {
                    Series series = (Series) description;
                    episode.setSeries(series);
                    // will be overwritten if the series is in a brand
                    episode.setContainer(series);
                    episode.setSeriesNumber(series.getSeriesNumber());
                    String seriesParentId = descriptionToParentId.get(series);
                    Identified seriesParent = idToDescription.get(seriesParentId);
                    if (seriesParent instanceof Brand) {
                        Brand brand = (Brand) seriesParent;
    ...
    public void processItemElement(Element itemElement) {
        ...
        if (mediaElement != null) {
            if (parentId.hasValue()) {
                ...
                log.record(new AdapterLogEntry(Severity.WARN).withSource(SeesawContentProcessor.class).withDescription("Ignoring SeeSaw top level item with uri " + getUri(itemElement)));
            }
        }
        else {
            if (parentId.hasValue()) {
