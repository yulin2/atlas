...
public class BbcSlashProgrammesTopicAdapter implements SiteSpecificAdapter<Topic> {
    ...
    private final ContentExtractor<SlashProgrammesRdf, Maybe<Topic>> topicExtractor;
    
    public BbcSlashProgrammesTopicAdapter(RemoteSiteClient<SlashProgrammesRdf> topicRdfClient, ContentExtractor<SlashProgrammesRdf, Maybe<Topic>> topicExtractor) {
        this.topicRdfClient = topicRdfClient;
        this.topicExtractor = topicExtractor;
    }
    
    @Override
    public Topic fetch(String uri) {
        checkArgument(canFetch(uri));
        try {
            SlashProgrammesRdf slashProgrammesTopic = topicRdfClient.get(uri + ".rdf");
            Maybe<Topic> extract = topicExtractor.extract(slashProgrammesTopic);
