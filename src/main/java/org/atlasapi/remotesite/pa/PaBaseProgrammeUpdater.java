...
public abstract class PaBaseProgrammeUpdater extends ScheduledTask {
    ...
    private static final DateTimeFormatter CHANNELINTERVAL_FORMAT = ISODateTimeFormat.dateTimeParser();
    ...
    public PaBaseProgrammeUpdater(PaChannelProcessor processor, PaProgrammeDataStore dataStore, ChannelResolver channelResolver, AdapterLog log) {
        this(new ThreadPoolExecutor(5, 10, 1, TimeUnit.MINUTES, new ArrayBlockingQueue<Runnable>(100), new ThreadFactoryBuilder().setNameFormat("pa-updater-%s").build(), new ThreadPoolExecutor.CallerRunsPolicy()), processor, dataStore, channelResolver, log);
    }
...
//                    if (channel.hasValue() && isSupported(channel.requireValue()) && shouldContinue()) {
    ...
    /**
     * The PA do not supply timezone information with times. Instead all times
     * for a file are consistently either GMT or BST,
     * based on the timezone of the listings_date. For example, for
     * listings_date of the Saturday of the switch from BST to
     * GMT, all times will be GMT+1h, even those after 0200. So 0500 London time
     * on the Sunday is really in GMT, but a
     * programme starting at 0500 in London will be listed as starting it 0600
     * (GMT+1)/
     *
     * @param date
     * @return
