...
public class S3LoveFilmFileUpdater implements LoveFilmFileUpdater {

    private static final String NO_DELIMITER = null;
    private static final Logger log = LoggerFactory.getLogger(S3LoveFilmFileUpdater.class);
    ...
    public S3LoveFilmFileUpdater(LoveFilmFileStore store, Supplier<S3Service> serviceSupplier, String bucket, String folder, String fileName) {
        this.store = store;
    ...
    public void update() {
        try {
            ...
            store.save(fileName, in.getInput());
        } catch (ServiceException e) {
            throw new FetchException(e.getMessage(), e);
        } catch (IOException e) {
            log.error("IO Exception when saving LoveFilm CSV file: " + e.getMessage(), e);
        } 
    }
