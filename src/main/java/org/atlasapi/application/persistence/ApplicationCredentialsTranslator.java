...
public class ApplicationCredentialsTranslator {
    ...
    public DBObject toDBObject(ApplicationCredentials credentials) {
        ...
        if (credentials.getIpAddressRanges() != null) {
            TranslatorUtils.fromList(dbo,
                    ImmutableList.copyOf(Iterables.transform(credentials.getIpAddressRanges(),
    ...
    public ApplicationCredentials fromDBObject(DBObject dbo) {
        ApplicationCredentials credentials = new ApplicationCredentials(
                TranslatorUtils.toString(dbo, API_KEY_KEY));

        return credentials.copyWithIpAddresses(Iterables.filter(Iterables.transform(TranslatorUtils.toList(dbo,
                IP_ADDRESS_KEY),
                TO_IP_RANGE),
                Predicates.notNull()));
    }
