...
public class ItemModelSimplifier extends ContentModelSimplifier<Item, org.atlasapi.media.entity.simple.Item> {
    ...
    @Override
    protected org.atlasapi.media.entity.simple.Item simplify(Item item) {
        return apply(item);
    }
    
    @Override
    public org.atlasapi.media.entity.simple.Item apply(Item full) {

        org.atlasapi.media.entity.simple.Item simple = new org.atlasapi.media.entity.simple.Item();
        simple.setType(EntityType.from(full).toString());

        for (Version version : full.getVersions()) {
            addTo(simple, version, full);
        }

        simple.setPeople(Iterables.filter(Iterables.transform(full.people(), crewSimplifier), Predicates.notNull()));

        copyProperties(full, simple);
    ...
    private SeriesSummary seriesSummaryFromResolved(ParentRef seriesRef) {
        // final Maybe<Identified> resolved =
        // contentResolver.findByCanonicalUris(ImmutableList.of(seriesRef.getUri())).get(seriesRef.getUri());
    ...
    private BrandSummary summaryFromResolved(ParentRef container) {
        // final Maybe<Identified> resolved =
        // contentResolver.findByCanonicalUris(ImmutableList.of(container.getUri())).get(container.getUri());
